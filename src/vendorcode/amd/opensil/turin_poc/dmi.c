...
void opensil_smbios_fill_cbmem_meminfo(void)
{
	...
	size_t dimm_cnt = 0;
	unsigned int skt, ch, dimm;
	unsigned int max_sockets = MIN(CONFIG_MAX_SOCKET, SIL_MAX_SOCKETS_SUPPORTED);
	bool empty;
	...
	/*
	 * Loop up to CONFIG_MAX_SOCKETS only.
	 * The array me not be filled for more sockets than 1 currently.
	 */
	for (skt = 0; skt < max_sockets; skt++) {
		for (ch = 0; ch < SIL_MAX_CHANNELS_PER_SOCKET; ch++) {
			for (dimm = 0; dimm < SIL_MAX_DIMMS_PER_CHANNEL; dimm++) {
				if (!mainboard_dimm_slot_exists(skt, ch, dimm))
					continue;

				empty = (dmi_info->T17[skt][ch][dimm].MemorySize == 0);

				printk(BIOS_DEBUG,
				       "Found %sDIMM on Socket %u Channel %u DIMM %u\n",
				       empty ? "empty ": "", skt, ch, dimm);

				if (!empty) {
					print_type17_info(&dmi_info->T17[skt][ch][dimm]);
					print_type20_info(&dmi_info->T20[skt][ch][dimm][0]);
				}
				...
				dimm_info->soc_num = skt;
				dimm_info->channel_num = ch;
				dimm_info->dimm_num = dimm;
				if (!empty) {
					transfer_memory_info(&dmi_info->T17[skt][ch][dimm], dimm_info);
...
int opensil_smbios_fill_dimm_locator(const struct dimm_info *dimm, struct smbios_type17 *t)
{
	...
	if (status != SilPass) {
		free(dmi_info);
		printk(BIOS_ERR, "Failed to obtain OpenSIL SMBIOS Mem Info\n");
		return -1;
	}
	...
	if (sizeof(type17->DeviceLocator) > (sizeof(locator) - 1)) {
		printk(BIOS_ERR,
		       "DIMM device locator buffer too small, increase locator size\n");
		free(dmi_info);
		return -1;
	}
	...
	if (sizeof(type17->BankLocator) > (sizeof(locator) - 1)) {
		printk(BIOS_ERR,
		       "DIMM bank locator buffer too small, increase locator size\n");
		free(dmi_info);
		return -1;
	}
